...
public final class TooManyRequestErrorHandler {

  private TooManyRequestErrorHandler() {
  ...
  public static void handleTooManyError( HttpServletResponse response, ConsumptionProbe consumptionProbe){
    ...
    response.addHeader(RateLimitHeader.X_RATE_LIMIT_RETRY_AFTER_SECONDS, String.valueOf(waitForRefill));
    try {
      response.sendError(HttpStatus.TOO_MANY_REQUESTS.value(),
          String.format("You have exhausted your API Request Quota, please try again in [%d] seconds.", waitForRefill));
    } catch (Exception ex) {
